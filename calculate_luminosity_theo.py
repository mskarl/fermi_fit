...
def get_luminosity(basepath, emin=100, emax=100000, z=0, verbose=False, z_correction=True):
    ...
    llh = np.load(os.path.join(basepath, 'llh.npy'),
                  allow_pickle=True, encoding='bytes')[()]
    source = llh[b'config'][b'selection'][b'target']
    if llh[b'sources'][source][b'ts'] < 4:#13.25
        try:
            gamma = llh[b'sources'][source][b'spectral_pars'][b'Index'][b'value']
            up_lim = llh[b'sources'][source][b'eflux100_ul95']/(1+z)**(2-gamma) * MeV_to_erg * z_surface_area
        except:
            up_lim =0
        return up_lim, 0, 0
    e2 = 10 ** (2 * bowtie[b'log_energies'])
    energies = np.array(10 ** bowtie[b'log_energies'])
    interp = InterpolatedUnivariateSpline(energies, bowtie[b'dnde'], k=1,ext=0)
    bf=quad(lambda x: interp(x/(1+z))*x/(1+z)**2, emin, emax)[0] * MeV_to_erg * z_surface_area
    interp = InterpolatedUnivariateSpline(energies, bowtie[b'dnde_lo'], k=1, ext=0)
    low = quad(lambda x: interp(x/(1+z))*x/(1+z)**2, emin, emax)[0] * MeV_to_erg * z_surface_area
    interp = InterpolatedUnivariateSpline(energies, bowtie[b'dnde_hi'], k=1, ext=0)
    up = quad(lambda x: interp(x/(1+z))*x/(1+z)**2, emin, emax)[0] * MeV_to_erg * z_surface_area
    if verbose:
        try:
            gamma = llh[b'sources'][source][b'spectral_pars'][b'Index'][b'value']
            bf2 = llh[b'sources'][source][b'eflux100']/(1+z)**(2-gamma) * MeV_to_erg * z_surface_area
